...
customerAddressRouter.post("/",expressAsyncHandler(async (req, res) => {
    ...
    res.send({ message: "Customer Address Saved", customerAddress: customerAddressSaved });
  })
);

customerAddressRouter.get('/customerAddList', expressAsyncHandler(async(req, res) => {
...
customerAddressRouter.put('/:id', isAuth, isSeller,isAdmin,isSellerOrAdmin, expressAsyncHandler(async(req, res) => {
  ...
  if (customAddressUpdate) {
    ...
    res.send({ message: "customerAddress Updated", customerAddress: UdatedcustomerAddress });
  } else {
...
customerAddressRouter.delete("/:id",expressAsyncHandler(async (req, res) => {
  ...
  if (deleteCustomAddress) {
    ...
    res.send({ message: "Customer Address Deleted", deleteCustomAddress: deleteCustomAddressDeleted });
  } else {
    res.status(404).send({ message: "Customer Address Not Found" });
  }
})
